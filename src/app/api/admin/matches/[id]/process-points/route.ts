...
export async function POST(
  ...
  try {
    ...
    if (snapshots.length === 0) {
      return NextResponse.json({ error: "Нет снапшотов для этого тура" }, { status: 400 });
    }
    ...
    // ── 3. Тренерский бонус: +2 если команда тренера победила ─────────────────
    ...
    await db.$transaction(async (tx) => {
      ...
      for (const snap of snapshots) {
        ...
        // Тренерский бонус (+2 если тренер победил)
